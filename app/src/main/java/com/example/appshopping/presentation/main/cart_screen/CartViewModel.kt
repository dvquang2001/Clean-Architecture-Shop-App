...
import com.example.appshopping.presentation.main.home.user_screen.UserInfoViewModel
...
class CartViewModel @Inject constructor(
...
) : BaseViewModel<CartViewModel.ViewState, CartViewModel.ViewEvent, CartViewModel.ViewEffect>(
    ViewState()
) {
    ...
    private fun pay(cartProductIds: List<String>, userAccountBalance: String, totalPrice: String) {
        ...
        payJob = coroutineScope.launch {
            payUseCase(cartProductIds, newUserAccountBalance.toString()).collect { result ->
                when (result) {
                    is ResultModel.Success -> setEffect(ViewEffect.PaySuccess)
    ...
    override fun onEvent(event: ViewEvent) {
        when (event) {
            ...
            is ViewEvent.Pay -> pay(
                event.cartProductIds,
                event.userAccountBalance,
                event.totalPrice
            )
        }
    }
    ...
    sealed interface ViewEvent : BaseViewEvent {
        ...
        data class Pay(
            val cartProductIds: List<String>,
            val userAccountBalance: String,
